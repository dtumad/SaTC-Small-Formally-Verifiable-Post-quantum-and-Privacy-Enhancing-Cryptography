...
% I'm a little unsure what "tense" this should all be written in? As compared to the paper
% e.g. "We next define a representation as..." vs. "VCVio then represents as..."
...
% I'm not sure if presenting this as a dichotomy is useful. It's more of a spectrum with some frameworks focusing more on some things
...
% Lean deprecated ⊥ even though it so many keystrokes
The singleton type \lean{Unit} has a single canonical element \lean{()}, and the empty type \lean{Empty} has no elements.
...
Before defining our model of computation we define a way to specify the set of oracles available to a computation via a structure \lean{OracleSpec ι}, where $\iota$ is an indexing set for available oracles, consisting of maps from indices to the domain and range types of the corresponding oracle:
\begin{leancode}
...
For instance we define \lean{singletonSpec T U} (Denoted \lean{T →ₒ U}) to represent access to
...
% This is sort of less true now that I'm bundling the security parameters rather than having two abstraction layers
...
% Can see that bundling s.p. in early makes definitions like this somewhat cluttered
% On the other hand other things later on end up being simpler
\begin{leancode}
...
\end{leancode}
% Can also use this for allowing small failure for message distributions
% Need to also modify experiment to draw m from mDist rather than take m as input
% \begin{leancode}
...
\label{sec:schnorr}
% In the code now Schnorr stuff is all done as sigma protocols.
% If I moved the sigma protocol stuff from later to this section, I could write this as "Forking Lemma" -> "Fiat-Shamir" -> Schnorr Sigma -> Schnorr signature
% Having all of that would probably push up page counts (although some sigma protocol stuff could leave the other section)
...
% "This can also be modeled as F-S applied to some specific sigma protocol..."?
% "Previous work has shown correctness of this sigma protocol (CITE), however doesn't implement a F-S to get the full signature"?
% "The main obstacle is the need for a forking lemma..."?
...
The full proof can be found in our \vcvio repository\cite{vcvio-github}.
% Should LOC here include e.g. stuff about generating seed values? Correspondence between input seeds and output logs?
% Depending on how you count could be from ~250 to ~1000
The proof draws on Mathlib~\cite{mathlib} machinery and consists of XXX lines of code.  (COMPARE TO PAPER VERSION)

(FORK FIGURE) % Is this a figure for the actual code?
...
% I still just have two fully complete reference impl for this table
% Can work on getting at least one or two more now that F-S/sigma-protocol stuff is done
